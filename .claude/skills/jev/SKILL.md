...
Der Cloud-Container hat Chromium, das Skript findet ihn unter `/opt/pw-browsers`. Drei Dinge muss der Nutzer in den Einstellungen der Umgebung freigeben: Netzwerkzugriff auf `openrouter.ai` (bei TypeSafe direkt zusätzlich `api.typesafe.ai`), Netzwerkzugriff auf die Seiten, die Jev öffnen soll, und den Schlüssel als Umgebungsvariable `OPENROUTER_API_KEY`. Einzelheiten stehen in `einrichtung.md`. Ohne Freigabe funktionieren `status` und `chrome` trotzdem, `inspect` nur bei erlaubten Seiten. Zeigt `inspect` als Titel nur den Hostnamen und keine Elemente, sperrt die Netzwerkregel die Seite; dann nicht weiter probieren, sondern dem Nutzer die Domain nennen. Dem Zertifikat des Cloud-Proxys vertraut Chromium über den Fingerabdruck, den das Skript beim Start mitgibt (`status` zeigt `proxy_ca`); zeigt eine Seite trotzdem `ERR_CERT_AUTHORITY_INVALID`, einmal `stop` und neu versuchen. Landet Jev auf einer Captcha- oder Bot-Sperrseite, nicht umgehen: dem Nutzer sagen, dass die Seite automatisierte Browser sperrt, und den lokalen Lauf vorschlagen.
